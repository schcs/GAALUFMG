...
:::{#def-}
Seja $W$ um subespaço de $\mathbb R^n$. Uma [base]{.bg-yellow} para $W$ é um sistema de geradores linearmente independentes para $W$ (nenhum vetor é combinação linear dos demais vetores).
:::
...
1. A base é ortogonal se quaisquer dois vetores da base são ortogonais ($\langle V_i,V_j\rangle=0$).
2. A base é ortonormal se todos os vetores são unitários e perpendiculares ($\langle V_i,V_j\rangle=0$ e $\langle V_i,V_i\rangle=1$).
