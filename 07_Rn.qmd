...
5. Sejam $V_1=(1,3,-2,5)$ e $V_2=(1,2,1,3)$ vetores de $\mathbb{R}^4$. Seja $W$ o conjunto dos vetores de $\mathbb{R}^4$ que são ortogonais a $V_1$ e a $V_2$.
    ...
    c. Se $V=(-9,-4,2,5)$, mostre que $V\in W$ e calcule $x_1$ e $x_2$ tais que $V=x_1V_1+x_2V_2$.
:::
...
Seja $W$ um subespaço de $\mathbb{R}^n$ de dimensão $k$. Sejam $\mathcal{B}=\{V_1,\ldots,V_k\}$ e $\mathcal{C}=\{U_1,\ldots,U_k\}$ duas bases de $W$. Então, para todo vetor $X \in W$, podemos escrever:
$$
...
A matriz de mudança de base de $\mathcal{B}$ para $\mathcal{C}$ é a matriz $P$ cujas colunas são as coordenadas dos vetores da base $\mathcal{C}$ na base $\mathcal{B}$:
$$P = [ [U_1]_{\mathcal{B}} \quad [U_2]_{\mathcal{B}} \quad \cdots \quad [U_k]_{\mathcal{B}} ].$$
Assim, se $[X]_{\mathcal{B}}$ e $[X]_{\mathcal{C}}$ são as matrizes coluna das coordenadas de $X$ nas bases $\mathcal{B}$ e $\mathcal{C}$, respectivamente, então
$$[X]_{\mathcal{B}} = P [X]_{\mathcal{C}}.$$
A matriz $P$ é invertível e sua inversa é a matriz de mudança de base de $\mathcal{C}$ para $\mathcal{B}$.
...
Determine a matriz de mudança de base de $\mathcal{B}$ para $\mathcal{C}$ e as coordenadas do vetor $X=(1,0,-1)$ na base $\mathcal{C}$.
...
\end{cases}$$
obtemos $U_1 = \frac{1}{3}V_1 + \frac{4}{3}V_2$ e $U_2 = \frac{5}{3}V_1 - \frac{2}{3}V_2$. Assim, a matriz de mudança de base de $\mathcal{B}$ para $\mathcal{C}$ é 
$$P = \begin{bmatrix} \frac{1}{3} & \frac{5}{3} \\[6pt] \frac{4}{3} & -\frac{2}{3} \end{bmatrix}.$$
:::

:::{.fragment .current-visible}
$$P = \begin{bmatrix} \frac{1}{3} & \frac{5}{3} \\[6pt] \frac{4}{3} & -\frac{2}{3} \end{bmatrix}.$$
Para encontrar as coordenadas de $X$ na base $\mathcal{C}$, primeiro encontramos as coordenadas de $X$ na base $\mathcal{B}$:
$$X = 0.5V_1 + 0.5V_2 \Rightarrow [X]_{\mathcal{B}} = \begin{bmatrix} 0.5 \\ 0.5 \end{bmatrix}.$$
Agora, usando a matriz de mudança de base, temos
$$[X]_{\mathcal{B}} = P [X]_{\mathcal{C}} \Rightarrow [X]_{\mathcal{C}} = P^{-1} [X]_{\mathcal{B}} = \begin{bmatrix} 1 \\ -0.5 \end{bmatrix}.$$
Portanto, as coordenadas de $X$ na base $\mathcal{C}$ são $(1, -0.5)$.
:::
:::
:::
