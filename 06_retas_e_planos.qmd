...
3. Considere o ponto $P=(4,-2,11)$ e a reta $r$ de equação 
$$ 
(x,y,z) = (1,-1,1) + t(3,1,2).
$$ 
Determine a equação da reta $s$ que passa por $P$ e que é perpendicular e concorrente com $r$.

## Exercício 3 -- solução

:::{.exr-}
Considere o ponto $P=(4,-2,11)$ e a reta $r$ de equação 
$$
(x,y,z) = (1,-1,1) + t(3,1,2). 
$$
Determine a reta $s$ que passa por $P$ e que é perpendicular e concorrente com $r$.
:::

O vetor diretor de $r$ é $V=(3,1,2)$. Procuramos um ponto $Q\in r$ tal que $\vec{PQ}\perp V$; ou seja, 
$\left<\vec{PQ}, V\right>=0$. 

:::{.r-stack}
:::{.fragment .current-visible}
$$
Q=(1,-1,1)+t(3,1,2)=(1+3t,-1+t,1+2t) \mbox{ e }\vec{PQ}=(-3+3t,1+t,-10+2t).
$$
Logo, 
$$
0=\left<\vec{PQ}, V\right>=\left<(-3+3t,1+t,-10+2t),(3,1,2)\right>=14t-28.
$$
Daí, $t=2$ e [$Q=(1,-1,1)+2(3,1,2)=(7,1,5)$]{.bg-yellow}.
:::

:::{.fragment .current-visible}
Logo $Q=(7,1,5)$ e um vetor diretor de $s$ é $\vec{PQ}=(3,3,-6)=3(1,1,-2)$. Assim, a reta procurada é
$$
(x,y,z)=(4,-2,11)+u(1,1,-2), u∈ℝ,
$$
com $\left<V,(1,1,-2)\right>=3+1-4=0$ (perpendicular) e concorrente em $Q$.
:::
:::
...
Temos 
$$
r: (x,y,z) = (3,5,3) + t(1,2,1) \mbox{ e } s: (x,y,z) = (1,2,3)+ u(1,2,1),\  t, u ∈ ℝ.
$$

[Distância entre duas retas paralelas.]{.bg-yellow}  

O vetor diretor das duas retas é $V=(1,2,1)$. 

Seja $A=(3,5,3)\in r$, $C=(1,2,3)\in s$ com $\vec{AC}=(-2,-3,0)$. Por Teorema de Pitágoras: 
$$
...
2. Mostre que $r$ e $s$ são retas concorrentes calculando o ponto $r \cap s$.
...
- $r: V_r = B-A = (1,-1,1) ⇒ (x,y,z) =  (4,-2,1) + t(1,-1,1)$.
- $s: V_s = D-C = (-8,-16,4)=-4(2,4,-1) ⇒ (x,y,z) = (1,7,4) + u(2,4,-1)$.

:::{.r-stack}
:::{.fragment .current-visible}
2. Posição relativa:
$V_r \not ∥ V_s$, logo as retas não são paralelas. Tentando $r ∩ s$:
\begin{align*}
4+t &= 1+2u\\ 
-2-t &= 7+4u \\
1+t &= 4-u 
...
:::{.fragment .current-visible}
[Distância e pontos de mínima distância:]{.bg-yellow}
Toma pontos genêricos $P\in r$ e $Q\in s$:
\begin{align*}
P&=(4+t,-2-t,1+t)\\
Q&=(1+2u,7+4u,4-u)\\
\vec{PQ}&=(-3+2u-t,9+4u+t,3-u-t).
\end{align*}
:::

:::{.fragment .current-visible}
Ora, queremos que $\vec{PQ}$ seja perpendicular a $V_r$ e a $V_s$:
\begin{align*}
0&=\left<\vec{PQ},V_r\right>=\left<(-3+2u-t,9+4u+t,3-u-t),(1,-1,1)\right>=-9-3u-3t\\
0&=\left<\vec{PQ},V_s\right>=\left<(-3+2u-t,9+4u+t,3-u-t),(2,4,-1)\right>=27+21u+3t
\end{align*}
:::
...
0&=27+21u+3t
\end{align*}
A solução do sistema é $t=-2$ e $u=-1$.
:::

:::{.fragment .current-visible}
Substituindo, $t=-2$ e $u=-1$,
\begin{align*}
P &= (4+t,-2-t,1+t)=  (2,0,-1)\\
Q &= (1+2u,7+4u,4-u)= (-1,3,5)\\
\vec{PQ} &= Q-P = (-3,3,6) \perp V_r, V_s.
\end{align*}

Distância:
$\|\vec{PQ}\|= \sqrt{9+9+36} = 3\sqrt 6$.
:::
:::
...
Nesta situação $N$ é um vetor [normal ao plano]{.bg-yellow}.
...
[Solução.]{.bg-yellow} A equação geral do plano é da forma $ax+by+cz=d$, em que os coeficientes $a$, $b$ e $c$ são as coordenadas de um vetor normal ao plano. Neste caso, a equação tem a forma
$$
5x-2y+3z=d.
$$ Substituindo o ponto $P_0=(2,3,-4)$, obtemos $d$:
...
- A equação geral do plano não é única: todo múltiplo não nulo de um vetor normal também é vetor normal. Multiplicar a equação 
$$
ax+by+cz=d
$$ 
por um número não nulo não altera o conjunto solução, apenas muda o vetor normal.
...
7x+5y-8z=d.
$$
O valor de $d$ é determinado por substitui um ponto do plano na equação. Usando $A$:
$$
7x+5y-8z=7\cdot (-1)+5\cdot 2-8\cdot 1=-5.
$$
A resposta final:
$$
7x+5y-8z=-5.
...
## Exemplo: Plano com três pontos

:::{#exm-}
Determine a equação geral do plano que contém
$$
A=(2,3,3),\quad B=(4,1,3),\quad C=(2,5,1).
$$
:::
...
4x+4y+4z=d.
$$ 
Substituindo $B=(4,1,3)$, $d=16+4+12=32$, ou seja, 
$$
4x+4y+4z=32,
$$ 
que simplifica para 
...
## Exemplo: plano dado por 3 pontos
...
:::{#exm-}
Verifique se as retas são paralelas, concorrentes ou reversas. Se possível, determine o plano que contém as duas retas.
...
1. Determine a equação paramétrica da reta $\overleftrightarrow{AB}$.
2. Calcule o ponto médio do segmento $AB$.
3. Calcule $\mathrm{dist}(A,B)$.
4. Determine o ponto simétrico $A'$ de $A$ em relação ao ponto $B$.
5. Determine o ponto simétrico $B'$ de $B$ em relação ao ponto $A$.
...
1.  O ponto $A$ pertence a reta $r$?
2.  Determine a equação do plano que contém $r$ e $A$.
3.  Determine a reta perpendicular a $r$ que passa por $A$.
4.  Calcule $\mathrm{dist}(A,r)$.
5.  Determine o ponto simétrico de $A$ em relação à reta $r$.
...
1.  O ponto $A$ pertence ao plano $\alpha$?
2.  Determine a equação paramétrica da reta que passa por $A$ e é perpendicular ao plano $\alpha$.
3.  Calcule $\mathrm{dist}(A,\alpha)$.
4.  Determine o ponto simétrico de $A$ em relação ao plano $\alpha$.
...
1. Determine a equação do plano $\alpha$ que contém $r$ e $s$.
2. Dê um exemplo de uma reta perpendicular a $r$ e a $s$.
3. Calcule $\mathrm{dist}(r,s)$.
4. Determine uma reta contida em $\alpha$ e que está equidistante de $r$ e de $s$.
...
1. Mostre que $r$ e $s$ são concorrentes calculando o ponto $P= r \cap s$.
2. Determine a equação geral do plano que contém $r$ e $s$.
3. Calcule o ângulo de $r$ e $s$.
...
1. Mostre que $r$ e $s$ são retas reversas.
2. Determine a equação da reta perpendicular e concorrente com $r$ e com $s$.
3. Calcule $\mathrm{dist}(r,s)$ e o ângulo entre $r$ e $s$.
4. Determine o plano $\alpha$ que contém $r$ e é paralelo a $s$.
5. Determine o plano $\beta$ que contém $s$ e é paralelo a $r$.
6. Determine o plano $\gamma$ que contém $r$ e é perpendicular a $\alpha$.
7. Determine o plano $\omega$ que contém $s$ e é perpendicular a $\beta$.
8. Determine a reta $\gamma \cap \omega$.


## Exemplo 6: ilustrações

:::{layout="[[33,33,33],[50,50]]"}
![](img/06_retas_e_planos/fig32.png){width=250}

![](img/06_retas_e_planos/fig33.png){width=250}


![](img/06_retas_e_planos/fig34.png){width=250}


![](img/06_retas_e_planos/fig35.png){width=250}


![](img/06_retas_e_planos/fig36.png){width=250}
:::
...
1. Determine o ponto $P = r \cap \alpha$.
2. Determine o plano que contém $r$ e é perpendicular a $\alpha$.
3. Determine a reta que é a projeção ortogonal de $r$ sobre $\alpha$.
4. Calcule o ângulo entre $r$ e $\alpha$.
5. Determine a reta contida em $\alpha$ e que é perpendicular a $r$.
...
1. Mostre que a reta $r$ é paralela ao plano $\alpha$.
2. Ache o plano que contém $r$ e é perpendicular a $\alpha$.
3. Ache o plano que contém $r$ e é paralelo a $\alpha$.
4. Determine a equação da reta que é a projeção ortogonal de $r$ sobre $\alpha$.
5. Calcule $\mathrm{dist}(r,\alpha)$.
...
1. Mostre que $r \subset \alpha$.
2. Determine o plano que contém $r$ e que é perpendicular a $\alpha$.
3. Dê um exemplo de uma reta contida em $\alpha$ e que é perpendicular a $r$.
...
1. Mostre que $\alpha$ e $\beta$ são planos paralelos.
2. Determine a reta perpendicular a $\alpha$ e a $\beta$ que passa pela origem.
3. Calcule $\mathrm{dist}(\alpha,\beta)$.
...
1. Mostre que $\alpha$ e $\beta$ não são paralelos.
2. Determine a equação da reta $\alpha \cap \beta$.
3. Calcule o ângulo entre $\alpha$ e $\beta$.
4. Dê um exemplo de um plano perpendicular a $\alpha$ e a $\beta$.
